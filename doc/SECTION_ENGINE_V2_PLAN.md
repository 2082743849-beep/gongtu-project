...
| 顺序 | 任务 | 难度 | 说明 |
|---:|---|---:|---|
| 1 | UX2-002 首屏压缩 | 低 | 只改布局，不碰 V2 数学与生命周期 |
| 2 | UX2-003 视角/切面拖拽状态机 | 高 | 指针事件和 OrbitControls 冲突是主要风险 |
| 3 | CUT-FIX-007 参考视频体验录制 | 中 | 验证肉眼连续性、构图、控件可达性 |
| 4 | COM-007 组合体恢复验收 | 中高 | 重点验证分层多 Mesh 接缝与多轮廓 |
| 5 | 5×5×5 分层搭建器 | 中高 | 复用现有积木数据和 V2，禁止另写截面算法 |
