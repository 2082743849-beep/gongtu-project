...
function validateSegments(segments) {
  ...
  segments.forEach((segment, index) => {
    ...
    for (const triangleId of segment.triangleIds ?? []) {
      if (!["string", "number"].includes(typeof triangleId)) {
        throw new TypeError(
          `segment ${index} triangleIds must contain only string or number values`,
        );
      }
      if (typeof triangleId === "number" && !Number.isFinite(triangleId)) {
        throw new RangeError(
          `segment ${index} numeric triangleIds must be finite`,
        );
      }
    }
  });
